...
if __name__ == '__main__':
    ...
    plt.errorbar(time_data, voltage_data, yerr=0.05, fmt=".", label="Capacitor Voltage", markersize=1,
                 elinewidth=0.2)
    ...
    # curves 1
    means = [draw_curve(-4.63, -3.265, time_data, voltage_data, RC_eqn_V_c),
    ...
    plt.savefig("RC_Exp1.png", dpi=250)
    plt.show()
    ...
    plot_residual(measured_data, calculated_data, x_axis_data, 0.05, "RC Capacitor voltage Residual(Fall)",
                  "Time (Sec)", "Voltage (V)")
    plt.show()

    # rise
    measured_data2, calculated_data2, x_axis_data2 = residual_stuff(time_data,
                                                                    voltage_data, means[1][0], means[1][2], -3.265,
                                                                    -2.046, RC_eqn_V_c_up)

    plot_residual(measured_data2, calculated_data2, x_axis_data2, 0.05, "RC Capacitor voltage Residual (rise)",
    ...
    # &&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
    print()
    draw_data("RC_extra_data_resistor.csv", "RC resistor ex2", "Time (Seconds)", "Voltage (Volts)",
              ["Resistor data", "curve_fit"], 0.05)
    time_data_1, voltage_data_1 = data_to_xy("RC_extra_data_resistor.csv")

    means = [draw_curve(-0.002499, -0.002004, time_data_1, voltage_data_1, RC_eqn_V_r),
             draw_curve(-0.001997, -0.001501, time_data_1, voltage_data_1, RC_eqn_V_r),
    ...
    plt.savefig('RC_resistor_ex2.png', dpi=250)
    plt.show()
    print()
    ...
    print("mean of v_0", v_mean / len(means))
    print("error of v_0 mean", 1 / len(means) * np.sqrt(v_error))
    print("mean of tau", (tau_mean / (10*len(means))))
    print("error of tau mean", 1 / len(means) * np.sqrt(tau_error))
    ##################################
    ...
    measured_data1, calculated_data1, x_axis_data1 = residual_stuff(time_data_1,
                                                                    voltage_data_1, means[0][0], means[0][2], -0.002499,
                                                                    -0.002004, RC_eqn_V_r)
    measured_data2, calculated_data2, x_axis_data2 = residual_stuff(time_data_1,
                                                                    voltage_data_1, means[1][0], means[1][2], -0.001997,
                                                                    -0.001501, RC_eqn_V_r)

    plot_residual(measured_data1, calculated_data1, x_axis_data1, 0.05, "RC resistor voltage Residual (rise)",
                  "Time (Seconds)", "Voltage (Volts)")
    plt.show()

    plot_residual(measured_data2, calculated_data2, x_axis_data2, 0.05, "RC resistor voltage Residual (drop)",
                  "Time (Seconds)", "Voltage (Volts)")
    plt.show()
