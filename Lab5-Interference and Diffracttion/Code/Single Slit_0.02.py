...
if __name__ == '__main__':
    ...
    plt.errorbar(xdata, ydata, fmt=".", markersize=1, elinewidth=0.2, yerr=y_uncert, alpha=0.1, label="Sensor Data")
    plt.plot(xdata, curve_data, label="Diffraction Curve")
    plt.xlabel("Location (meters)")
    plt.ylabel("Intensity (Volts)")
    plt.axhline(y=0)
    plt.legend()
    plt.savefig("Pics/Single_Slit_0.02.png", dpi=500)
    plt.show()
    ...
    plt.axhline(y=0)
    plt.legend()
    plt.savefig("Pics/Single_Slit_0.02_residual.png", dpi=500)
    plt.show()
